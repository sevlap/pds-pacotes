...
import com.brunoalves.pacotes.repositorio.ItemRepositorioCustom;

@Repository
@Transactional(readOnly=true)
public class ItemRepositorioImpl implements ItemRepositorioCustom {
	...
	public Item buscarNomeExato(String nome) {
		String jpql = "SELECT x FROM Item x WHERE x.nome = :p1";
	...
	public List<Item> buscarTodosOrdenadosPorNome() {
		String jpql = "SELECT x FROM Item x ORDER BY x.nome";
		Query query = em.createQuery(jpql);
	...
	public List<Item> buscarPorNome(String trecho) {
		String jpql = "SELECT x FROM Item x WHERE x.nome LIKE :p1";
		Query query = em.createQuery(jpql);
